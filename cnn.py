################Final experiment of ANER using bidirectional lstm recurrent neural networks#################
...
from keras.layers import LSTM,Embedding,Bidirectional,TimeDistributed,Input,merge,Conv1D,MaxPooling1D
...
for index in range(0,10) :
     ...
     Embedding_layer = Embedding(input_dim=4600, output_dim = 100, input_length=seq_length)(Input_layer)
     cnn1=Conv1D(filters=32, kernel_size=3, padding='same', activation='relu')(Embedding_layer)
     cnn2=MaxPooling1D(pool_size=2)(cnn1)
     Recurrent_layer=Bidirectional(LSTM(100,return_sequences=True))(cnn2)
     Output_layer = TimeDistributed(Dense(5, activation='softmax'))(Recurrent_layer)
     # create the model
